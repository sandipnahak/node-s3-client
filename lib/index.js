...
Client.prototype.uploadFile = function(params) {
  ...
  var sseType = s3Params.ServerSideEncryption;
  ...
  if (sseType === undefined) {
    getDefaultBucketEncryption(s3Params.Bucket);
  } else {
    checkSseType();
  }
  ...
  function checkSseType() {
    isKMS = Boolean(sseType === KMS_SSE_TYPE)
    openFile();
  }

  function getDefaultBucketEncryption(bucket) {
    var params = {
      Bucket: bucket
    };
    var encryption = self.s3.getBucketEncryption(params, function(err, data) {
      if (!err) { 
        var defaultEncryption = data.ServerSideEncryptionConfiguration.Rules[0]
        var applyByDefault = defaultEncryption.ApplyServerSideEncryptionByDefault;
        if (applyByDefault !== undefined) {
          sseType = applyByDefault.SSEAlgorithm;
        }
      }
      else {
    ...
    function tryUploadPart(cb) {
      ...
      self.s3Pend.go(function(pendCb) {
        ...
        self.s3.uploadPart(extend({}, s3Params), function(err, data) {
          ...
          pend.wait(function() {
            if (fatalError) return;
            if (!isKMS && !compareMultipartETag(data.ETag, multipartETag)) {
              errorOccurred = true;
              uploader.progressAmount -= overallDelta;
    ...
    self.s3Pend.go(function(pendCb) {
      ...
      self.s3.putObject(s3Params, function(err, data) {
       ...
       gotCallback = true;
        ...
        pend.wait(function() {
          if (fatalError) return;
          if (!isKMS && !compareMultipartETag(data.ETag, localFileStat.multipartETag)) {
            cb(new Error("ETag does not match MD5 checksum"));
            return;
          }
          cb(null, data);
      ...
      if (headers['content-length'] == undefined) {
        ...
        hashCheckPend.go(function(cb) {
          multipartETag.on('end', function() {
            ...
            if (headers['x-amz-server-side-encryption'] !== KMS_SSE_TYPE) {
...
Client.prototype.downloadBuffer = function(s3Params) {
  ...
  var sseType = s3Params.ServerSideEncryption;
  var isKMS = Boolean(sseType == 'aws:kms')
  downloader.progressAmount = 0;
